...
TOPICS = {"recommendation_results": "recommendation_results"}
...
def kafka_topic_updater(
    topic_key, topic_name, value_format="avro", poll_interval=1.0, max_messages=50
):
...
RECO_SCHEMA_PATH = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), "../schemas/user_recommendation_requests.avsc"
    )
)
...
schema_registry_client = SchemaRegistryClient(SCHEMA_REGISTRY_CONFIG)
reco_avro_serializer = AvroSerializer(
    schema_registry_client, reco_schema_str, lambda obj, ctx: obj
)
...
def send_recommendation_request(req: RecommendationRequest):
    ...
    reco_producer.produce(
        topic=RECO_TOPIC,
        key=reco_string_serializer(
            user_id, SerializationContext(RECO_TOPIC, MessageField.KEY)
        ),
        value=reco_avro_serializer(
            msg_dict, SerializationContext(RECO_TOPIC, MessageField.VALUE)
        ),
    )
    reco_producer.flush()
...
def get_message_by_request_id(request_id: str):
    ...
    raise HTTPException(
        status_code=404, detail="Message not found for the given request_id"
    )
