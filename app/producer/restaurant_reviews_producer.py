...
class RestaurantReview(object):
    ...
    def __init__(
        ...
        restaurant_id,
        restaurant_name,
        food_items_mentioned_in_review,
        review_text,
        user_id,
        rating,
    ):
...
def delivery_report(err, msg):
    ...
    if err is not None:
        ...
        return
    print(
        "Review record with Id {} successfully produced to Topic:{} Partition: [{}] at offset {}".format(
...
def main():
    ...
    avro_serializer = AvroSerializer(schema_registry_client, schema_str, review_to_dict)
        ...
        for column in reader:
            ...
            producer.produce(
                ...
                key=string_serializer(
                    str(review.restaurant_id),
                    SerializationContext(topic=topic, field=MessageField.KEY),
                ),
                value=avro_serializer(
                    review, SerializationContext(topic, MessageField.VALUE)
                ),
